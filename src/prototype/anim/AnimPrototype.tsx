  ...
  return (
    ...
    >
      <div style={{ maxWidth: 860, margin: '0 auto' }}>
        ...
        <p style={{ margin: '0 0 40px', fontSize: 13, color: C.muted, lineHeight: 1.6, maxWidth: 520 }}>
          Variant{' '}
...
function DemoRing({
  ...
  const c = size / 2
  const breatheAnim = rm || !running
    ? 'none'
    : `protoBreathe ${timing.ring.dur} ${timing.ring.easing} infinite`
  return (
    // key forces animation restart when variant changes
    <div
      key={`ring-${timing.key}`}
      style={{ position: 'relative', width: size, height: size, display: 'grid', placeItems: 'center', flex: '0 0 auto' }}
    >
      <svg
        width={size}
        height={size}
      ...
      >
        ...
        <circle
          ...
          style={{ transition: `stroke-dashoffset ${rm ? '0s' : timing.progress.dur} ${timing.progress.easing}` }}
        />
      </svg>
    </div>
  )
...
function HoverDemo({ timing, rm }: { timing: TimingSet; rm: boolean }) {
  const pillTrans = rm
    ? 'none'
    : `transform ${timing.pill.hoverDur} ${timing.pill.hoverEasing}`
  ...
  return (
    <div style={{ display: 'flex', gap: 48, flexWrap: 'wrap', alignItems: 'flex-start' }}>
      {/* Island pill hover */}
      <div>
        <div style={{ fontFamily: C.MONO, fontSize: 9, color: C.faint, marginBottom: 10, letterSpacing: '0.1em' }}>
          PILL HOVER-LIFT
        </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
      ...
      >
        ...
        >
          <span style={{ fontFamily: C.MONO, fontSize: 9, color: C.accent, letterSpacing: '0.12em' }}>
            DONE
          </span>
        </div>
      ...
      <div style={{ fontFamily: C.MONO, fontSize: 9, color: C.faint, textAlign: 'center', lineHeight: 1.6 }}>
        {rm ? (
          'static (rm)'
        ) : (
          <>
...
function UrgentDemo({ timing, rm }: { timing: TimingSet; rm: boolean }) {
  ...
  const accentTrans = rm ? 'none' : `background ${timing.urgent.dur}, box-shadow ${timing.urgent.dur}`
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, maxWidth: 420 }}>
      <div style={{ display: 'flex', gap: 24, alignItems: 'center' }}>
        ...
        {/* Time label */}
        <div style={{ fontFamily: C.MONO, fontSize: 36, fontWeight: 500, transition: `color ${rm ? '0s' : timing.urgent.dur}`, color: urgent ? C.amber : C.text }}>
          00:47
        </div>
        {/* Status label */}
        <div
  ...
  return (
    ...
    >
      ...
      <circle
        ...
        strokeDashoffset={offset}
        style={{
          transition: rm ? 'none' : `stroke ${timing.urgent.dur}, stroke-dashoffset ${timing.progress.dur} ${timing.progress.easing}`,
        }}
      />
    </svg>
  )
...
function TimingTable({ timing, rm }: { timing: TimingSet; rm: boolean }) {
  const rows: [string, string, string][] = [
    ...
    ['Ring breathe scale', String(timing.ring.scale), rm ? '1 (static)' : String(timing.ring.scale)],
    ...
    ['Primary btn hover scale', String(timing.btn.hoverScale), rm ? '1' : String(timing.btn.hoverScale)],
    ['Primary btn active scale', String(timing.btn.activeScale), rm ? '1' : String(timing.btn.activeScale)],
  ...
  return (
    <div style={{ overflowX: 'auto' }}>
      <table style={{ borderCollapse: 'collapse', fontSize: 12, fontFamily: C.MONO, width: '100%' }}>
        <thead>
          <tr>
...
function VariantBar({
  ...
  return (
    <div style={pill}>
      ...
      {/* Keyboard hint */}
      <span style={{ fontFamily: C.MONO, fontSize: 9, color: C.faint, padding: '0 4px', whiteSpace: 'nowrap' }}>
        ← →
      </span>
    </div>
  )
